...
int main(void) {
  // 1. Enable debugging/memory read by other processes (bypasses Yama
  // ptrace_scope = 1)
  if (prctl(PR_SET_PTRACER, PR_SET_PTRACER_ANY, 0, 0, 0) == -1) {
    perror("[GAME] Warning: prctl(PR_SET_PTRACER) failed. Quark daemon might "
           "fail to read memory");
  } else {
    printf(
        "[GAME] prctl(PR_SET_PTRACER, PR_SET_PTRACER_ANY) set successfully.\n");
  }

  // 2. Initialize Quark SDK
  printf("[GAME] Connecting to Quark Anticheat...\n");
  if (quark_sdk_init() == -1) {
    printf("[GAME] WARNING: Quark Anticheat Daemon not detected. Running "
           "without protection.\n");
  } else {
    ...
    printf(
        "[GAME] Monitored variables registered with Quark Anticheat Daemon.\n");
  }
  ...
  char choice;
  while (1) {
    printf("\nCommands: [h] Take Damage (-10 Health), [s] Add Score (+100), "
           "[p] Print Status, [q] Quit\n");
    printf("Enter command: ");
    if (fflush(stdout) != 0) {
      perror("[GAME] fflush(stdout)");
      break;
    }
